...
describe('cookieOwnsSession', () => {
  /**
   * The one fact the post-call sign-up rests on: a password may be set on the
   * account a guest session becomes ONLY when the browser asking is the one the
   * server opened that session for. A bare session id is not that proof — it is
   * in a URL, and URLs get pasted.
   */
  it('says yes only for a session the cookie actually carries', () => {
    expect(cookieOwnsSession(cookieWith(), SESSION)).toBe(true)
    expect(cookieOwnsSession(cookieWith('another-session'), SESSION)).toBe(false)
    expect(cookieOwnsSession(null, SESSION)).toBe(false)
    expect(cookieOwnsSession(cookieWith(), '')).toBe(false)
  })

  it('still says yes once the consultation is over', () => {
    // It is asked AFTER the call, so "finished" is the normal case. Nothing
    // about the row, the clock or the mint cooldown belongs in this answer —
    // those are the mint's business, and the mint has its own nine rules.
    const yesterday: ReturnType<typeof cookieWith> = {
      g: 'guest-1',
      s: [{ i: SESSION, c: NOW_S - 23 * 60 * 60, m: NOW_S - 23 * 60 * 60 }],
    }
    expect(cookieOwnsSession(yesterday, SESSION)).toBe(true)
  })

  it('reads through a real signed cookie, which is how a route gets one', () => {
    const signed = signGuestCookie(withGuestSession(null, SESSION, NOW_S))!
    expect(cookieOwnsSession(readGuestCookie(signed), SESSION)).toBe(true)
    expect(cookieOwnsSession(readGuestCookie('forged.nonsense'), SESSION)).toBe(false)
  })
})
