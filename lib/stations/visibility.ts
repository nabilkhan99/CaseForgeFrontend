 ...
 * live bank. NEXT_PUBLIC_ because some station queries run in the browser;
 * the value is baked in per-deployment at build time. Never set it on the
 * Production environment: activating a station for real is a DB flip
 * (`is_active = true`), not this switch.
 */
