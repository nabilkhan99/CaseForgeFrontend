...
function getDomainIcon(domainName: string) {
    ...
    const svg = (children: React.ReactNode) => (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className={cls}>
            {children}
        </svg>
    );
    ...
    // Allergy — flower/pollen
    if (name.includes('allergy'))
        return svg(<><circle cx="12" cy="12" r="3" /><path d="M12 2v4" /><path d="M12 18v4" /><path d="M4.93 4.93l2.83 2.83" /><path d="M16.24 16.24l2.83 2.83" /><path d="M2 12h4" /><path d="M18 12h4" /><path d="M4.93 19.07l2.83-2.83" /><path d="M16.24 7.76l2.83-2.83" /></>);
    // Dermatology — lotion bottle
    if (name.includes('dermatology'))
        return svg(<><rect x="6" y="10" width="12" height="12" rx="2" /><path d="M10 10V7a2 2 0 0 1 2-2v0a2 2 0 0 1 2 2v3" /><path d="M10 4h4" /><line x1="10" y1="15" x2="14" y2="15" /></>);
    if (name.includes('mental health'))
        return svg(<><path d="M12 2a9 9 0 0 1 9 9c0 3.47-1.96 6.48-4.84 8.01L15 21H9l-.16-1.99A9.001 9.001 0 0 1 3 11a9 9 0 0 1 9-9z" /><line x1="9" y1="11" x2="9.01" y2="11" /><line x1="12" y1="9" x2="12.01" y2="9" /><line x1="15" y1="11" x2="15.01" y2="11" /></>);
    // Neurology — neuron shape
    if (name.includes('neurology') || name.includes('neurodiversity'))
        return svg(<><circle cx="12" cy="12" r="3" /><path d="M12 2v7" /><path d="M12 15v7" /><path d="M4.93 4.93l4.95 4.95" /><path d="M14.12 14.12l4.95 4.95" /><path d="M2 12h7" /><path d="M15 12h7" /><path d="M4.93 19.07l4.95-4.95" /><path d="M14.12 9.88l4.95-4.95" /></>);
    // Respiratory — two lungs
    if (name.includes('respiratory'))
        return svg(<><path d="M6.5 18C4 18 2 16 2 13.5c0-2 1-3.5 2.5-4.5L7 7V3h4v9" /><path d="M17.5 18C20 18 22 16 22 13.5c0-2-1-3.5-2.5-4.5L17 7V3h-4v9" /></>);
    // Musculoskeletal — bone
    if (name.includes('musculoskeletal'))
        return svg(<><path d="M8 4a2 2 0 0 0-2 2c0 .74.4 1.39 1 1.73V12" /><path d="M16 4a2 2 0 0 1 2 2c0 .74-.4 1.39-1 1.73V12" /><path d="M8 20a2 2 0 0 1-2-2c0-.74.4-1.39 1-1.73V12" /><path d="M16 20a2 2 0 0 0 2-2c0-.74-.4-1.39-1-1.73V12" /><line x1="7" y1="12" x2="17" y2="12" /></>);
    ...
    if (name.includes('gastroenterology'))
        return svg(<path d="M16 4c2 0 4 1.5 4 4s-1 4-3 5c-1.5.75-2 2-2 3v2c0 2-1.5 3-3 3s-3-1-3-3c0-2-.5-3-2-4S4 11 4 8c0-2.5 2-4 4-4h8z" />);
    if (name.includes('ent') || name.includes('ear') || name.includes('nose'))
        return svg(<><path d="M6 8.5a6.5 6.5 0 1 1 13 0c0 6-6 6-6 10a3.5 3.5 0 0 1-7 0" /><path d="M15 8.5a2.5 2.5 0 0 0-5 0v1a2 2 0 0 1-2 2" /></>);
    if (name.includes('haematology'))
        return svg(<path d="M12 2 C12 2 5 9 5 14 a7 7 0 0 0 14 0 C19 9 12 2 12 2z" />);
    // Renal & urology — two kidneys with ureters
    if (name.includes('renal') || name.includes('urology'))
        return svg(<><path d="M5 8c-1 1-2 3-2 5s1 4 2 5c.5.5 1.5 1 2.5 0 .5-.5 1-2 1-4V8c0-2-.5-3.5-1-4C7 3.5 6 4 5 5" /><path d="M19 8c1 1 2 3 2 5s-1 4-2 5c-.5.5-1.5 1-2.5 0-.5-.5-1-2-1-4V8c0-2 .5-3.5 1-4 .5-.5 1.5-1 2.5 0" /><path d="M7.5 18v3h2" /><path d="M16.5 18v3h-2" /></>);
    // Maternity — pregnant woman stick figure
    if (name.includes('maternity') || name.includes('reproductive'))
        return svg(<><circle cx="10" cy="4" r="2.5" /><path d="M10 6.5v4" /><path d="M10 10.5c2.5 0 4 1.5 4.5 4" /><path d="M7 22l3-7.5" /><path d="M13 22l-3-7.5" /></>);
    // Sexual health — concentric circles (condom)
    if (name.includes('sexual health'))
        return svg(<><circle cx="12" cy="12" r="8" /><circle cx="12" cy="12" r="4" /></>);
    // Genetics — DNA strand
    if (name.includes('genetics'))
        return svg(<><path d="M2 15c6.667-6 13.333 0 20-6" /><path d="M9 22c1.798-1.998 2.518-3.995 2.807-5.993" /><path d="M15 2c-1.798 1.998-2.518 3.995-2.807 5.993" /><path d="m2 9 20 6" /><path d="m2 21 20-6" /></>);
    ...
    if (name.includes('older adult'))
        return svg(<><circle cx="12" cy="5" r="2" /><path d="M12 7v6" /><path d="M9 10l-2 7" /><path d="M15 10l2 7" /><path d="M7 17l-1 4" /><path d="M17 17l1 4" /></>);
    // End of life — tombstone
    if (name.includes('end of life'))
        return svg(<><path d="M6 22V10a6 6 0 0 1 12 0v12" /><line x1="6" y1="22" x2="18" y2="22" /><line x1="12" y1="14" x2="12" y2="18" /><line x1="10" y1="16" x2="14" y2="16" /></>);
    if (name.includes('infectious'))
        return svg(<><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /><circle cx="12" cy="11" r="2" /><path d="M12 9v-2" /><path d="M12 13v2" /><path d="M10 11H8" /><path d="M14 11h2" /></>);
    // Metabolic/Endocrinology — fire with circular arrows
    if (name.includes('metabolic') || name.includes('endocrinology'))
        return svg(<><path d="M12 22c-4 0-6-3-6-6 0-2 1-4 3-5.5C10 9.5 10 8 9.5 6c2 1 3 3 3.5 5 .5-1 1-2 1-3.5C16 9 18 11.5 18 16c0 3-2 6-6 6z" /><path d="M19 12a7 7 0 0 0-1-3.5" /><path d="M20.5 14l1.5-2-2-1" /><path d="M5 12a7 7 0 0 1 1-3.5" /><path d="M3.5 14l-1.5-2 2-1" /></>);
    // Gynaecology — Venus symbol (♀)
    if (name.includes('gynaecology'))
        return svg(<><circle cx="12" cy="9" r="5" /><path d="M12 14v8" /><path d="M9 19h6" /></>);
    ...
    // Urgent/unscheduled — medical cross (thicker)
    if (name.includes('urgent') || name.includes('unscheduled'))
        return (
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={cls}>
                <line x1="12" y1="4" x2="12" y2="20" />
                <line x1="4" y1="12" x2="20" y2="12" />
            </svg>
        );
    // Default: stethoscope
    return svg(<><path d="M4.8 2.3A.3.3 0 1 0 5 2H4a2 2 0 0 0-2 2v5a6 6 0 0 0 6 6v0a6 6 0 0 0 6-6V4a2 2 0 0 0-2-2h-1a.2.2 0 1 0 .3.3" /><path d="M8 15v1a6 6 0 0 0 6 6v0a6 6 0 0 0 6-6v-4" /><circle cx="20" cy="10" r="2" /></>);
}
